...
class Utils {
  ...
  static getRouteInfo(navTree, path) {
    if (navTree.path === path || path.includes(navTree?.path)) {
      return navTree;
    }
    let route;
    for (let i = 0; i < navTree.length; i++) {
      if (typeof navTree[i] === 'object') {
        route = this.getRouteInfo(navTree[i], path);
        if (route) {
          return route;
        }
      }
...
export const getPercentage = (num1, num2) => {
  return Math.floor(100 * (num2 / num1));
};
export const getDurationHHmm = (timeString) => {
  ...
  const value = dayjs.duration({ hours: hInt, minutes: mInt }).humanize();
  return value;
};
export const relativeTime = (timeString) => {
  return dayjs(timeString).fromNow();
};
export const getProfileUrl = (role_id, id) => {
  // role_id|1 -> admin 2 -> instructor 3 -> student
  const url =
    role_id === 2
      ? `/instructors/${id}`
      : role_id === 3
...
export const isTextEditorValue = (text) => {
  const div = document.createElement('div');
  div.innerHTML = text.trim();
  const value = div?.firstChild?.innerHTML?.trim();
  if (!value || value === '<br>') {
    return null;
  }
  return text;
